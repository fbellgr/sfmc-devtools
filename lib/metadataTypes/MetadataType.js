...
class MetadataType {
    ...
    static paginate(url, last) {
        if (this.definition.restPagination) {
            ...
            // Maximum page size per object is inconsistent in the undocumented REST API
            queryParams.set('$pagesize', '2500');
            // if no page add page
    ...
    static async saveResults(results, retrieveDir, overrideType, templateVariables) {
        ...
        for (const originalKey in results) {
            try {
                ...
                const assetType = results[originalKey].assetType?.name;
                const subType = this.definition.extendedSubTypes
                    ? (() => {
                          for (const subType of Object.keys(this.definition.extendedSubTypes)) {
                              if (
                                  this.definition.extendedSubTypes[subType].indexOf(assetType) >= 0
                              ) {
                ...
                const subtypeExtension =
                    '.' + (overrideType || subType || this.definition.type) + '-meta';
                // define directory into which the current metdata shall be saved
                const baseDir = [
                    retrieveDir,
                    ...(overrideType || subType || this.definition.type).split('-'),
                ];
