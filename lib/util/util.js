...
const Util = {
     ...
     * @param {...object} [parts] list of arguments
     * @returns {object} the resulting filter
     */
    joinFilterParts(logicalOperator, ...parts) {
        if (!/^(or|and$)/i.test(logicalOperator)) {
            throw new Error(
                `logicalOperator - Invalid parameter: ${logicalOperator || 'undefined'}`
            );
        }
        if (_.isArray(parts)) {
            if (parts.length === 1) {
            ...
            const leftOperand = parts.shift();
            const filter = {
                leftOperand,
                logicalOperator: logicalOperator,
                rightOperand: Util.joinFilterParts(logicalOperator, ...parts),
            };

            return filter;
        }
        throw new Error(`parts - Invalid parameter`);
    },
};
/**
