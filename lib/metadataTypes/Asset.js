    ...
    static async requestSubType(
        ...
        if (templateName) {
            options.json.query = {
                leftOperand: {
                    ...
                    simpleOperator: subtypeIds.length === 1 ? 'equals' : 'in',
                    value: subtypeIds.length === 1 ? subtypeIds[0] : subtypeIds,
                },
                logicalOperator: 'AND',
                rightOperand: {
                    property: 'name',
        ...
        do {
            ...
            if (response && response.body && response.body.items && response.body.items.length) {
                ...
                Util.logger.info(`Downloaded: ${subType} (${items.length}/${response.body.count})`);
            }
        ...
        // filter assets that have keys with trailing spaces, depending on the type
        const excluded = [];
        items = items.filter((item) => {
            if (/^\s|\s$/.test(item.customerKey)) {
                Util.logger.warn(
                    `Trailing or leading space - ${subType} with customer='${item.customerKey}'`
                );
                if (
                    /\s$/.test(item.customerKey) &&
                    /^(template|webpage|templatebasedemail|htmlemail)$/.test(item.assetType.name)
                ) {
                    Util.logger.warn(
                        `Excluding ${item.assetType.name} with customer='${item.customerKey}'`
                    );
                    excluded.push(item);
                    return false;
                }
    ...
    static _extractCode(metadata) {
        ...
        switch (metadata.assetType.name) {
            case 'template': // template
                ...
                // metadata.views.html.slots.<>.blocks.<>.content (optional)
                if (metadata.slots) {
                    this._extractCode_slots(metadata.slots, codeArr);
                }
